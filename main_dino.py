...
def get_args_parser():
    ...
    parser.add_argument('--momentum_teacher', default=0.9996, type=float, help="""Base EMA
    ...
    parser.add_argument('--teacher_temp', default=0.05, type=float, help="""Final value (after linear warmup)
    ...
    parser.add_argument('--freeze_last_layer', default=15, type=int, help="""Number of epochs
...
def train_dino(args):
    ...
    if args.arch in vits.__dict__.keys():
        ...
        teacher.patch_embed.proj = torch.nn.Conv2d(
            in_channels=1,
            out_channels=embed_dim,
            kernel_size=(args.patch_size, args.patch_size),
            stride=(args.patch_size, args.patch_size)
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        student = torchvision_models.__dict__[args.arch](pretrained=False)
        teacher = torchvision_models.__dict__[args.arch](pretrained=False)
    ...
    to_restore = {"epoch": 0}
    # checkpoint_path = "checkpoint/dino_vitbase8_pretrain_full_checkpoint.pth"
    # checkpoint_path = "checkpoint/dino_vitbase8_pretrain.pth"
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        if fp16_scaler is None:
            ...
            if args.clip_grad:
                fp16_scaler.unscale_(optimizer)  # unscale the gradients of optimizer's asgned params in-place
                param_norms = utils.clip_gradients(student, args.clip_grad)
            utils.cancel_gradients_last_layer(epoch, student,
                                              args.freeze_last_layer)
...
def open_image(path):
    ...
    image_tensor = torch.from_numpy(image).unsqueeze(dim=0)
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        for iq, q in enumerate(teacher_out):
            for v in range(len(student_out)):
                ...
                # print(q)
                # print(student_out[v])
                # print(F.log_softmax(student_out[v], dim=-1))
                loss = torch.sum(-q * F.log_softmax(student_out[v], dim=-1), dim=-1)
                loss_item = loss.mean()
                total_loss += loss_item
                # print(f"loss item {loss_item} total loss {total_loss}")
                n_loss_terms += 1
...
class DataAugmentationDINO_reduced(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        flip_tranforms = transforms.Compose([
            transforms.RandomHorizontalFlip(p=0.5),
        ...
        rotation_transform = transforms.Compose([
            transforms.RandomApply(
                [transforms.RandomRotation([60, 150])],
                p=0.3
            )
        ])
